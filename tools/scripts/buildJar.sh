...
buildJar() {
    ...
    cd "tools/Paperclip"
    mvn clean package "-Dmcver=$mcVersion" "-Dpaperjar=$rootPath/server/build/libs/loom-server-$mcVersion.jar" "-Dvanillajar=$rootPath/.cache/$mcVersion/server.jar" || exit 1

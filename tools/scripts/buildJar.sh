...
buildJar() {
    ...
    cd "tools/Paperclip"
    "$rootPath/mvnw" clean package "-Dmcver=$mcVersion" "-Dpaperjar=$rootPath/server/target/loom-$mcVersion.jar" "-Dvanillajar=$rootPath/.cache/$mcVersion/server.jar" || exit 1
